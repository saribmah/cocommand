...
- Extensible workflows and integrations.
...
3) Plan: planner selects workflows, tools, and steps.
...
- Workflow: reusable, parameterized multi-step actions.
...
- packages/workflows: workflow registry, schema, validation.
...
## Workflow Model
Workflows are declarative steps with typed inputs/outputs.
- Each step maps to a tool provided by integrations.
- LLM can fill parameters but execution is deterministic.
- User-defined workflows are validated against schema.

## Execution Safety
- Default: read-only unless user confirms.
- Tiered permissions per workflow and tool.
- Audit log for actions and results.

## Extensibility
- Integrations expose tool definitions and capabilities.
- Workflows compose tools with explicit inputs/outputs.
- Planner chooses workflows/tools based on intent and context.
