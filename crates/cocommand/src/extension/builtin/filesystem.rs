...
// Use filesystem crate for indexing
use filesystem::FileSystemIndexManager;

use self::ops::{list_directory_entries, path_info, read_file_content};
use self::platform::{open_path_native, reveal_path_native};
use self::types::{EntryKindFilter, ListSortKey, SortOrder};
        ...
        let search_execute = Arc::new(
            move |input: serde_json::Value, context: crate::extension::ExtensionContext| {
                ...
                boxed_tool_future(async move {
                    ...
                    .await?;
                    let payload: types::SearchPayload = result.into();
                    Ok(json!(payload))
                })
            },
        );
        ...
        let index_status_execute = Arc::new(
            move |input: serde_json::Value, context: crate::extension::ExtensionContext| {
                ...
                boxed_tool_future(async move {
                    ...
                    .await?;
                    let payload: types::IndexStatusPayload = result.into();
                    Ok(json!(payload))
                })
            },
        );
...
fn parse_search_request_options(input: &serde_json::Value) -> CoreResult<SearchRequestOptions> {
...
fn normalize_input_path(raw: &str, workspace_dir: &Path) -> CoreResult<PathBuf> {
