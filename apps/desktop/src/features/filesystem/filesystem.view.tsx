...
export function FileSystemView({ mode, actions }: ExtensionViewProps) {
  ...
  return (
    <div className={`${isInline ? "" : "app-shell "}${styles.container}${isInline ? ` ${styles.inline}` : ""}`}>
      <FileList
        ...
        onActivate={(entry) => actions?.addPart({ type: "file", path: entry.path, name: entry.name, entryType: entry.type })}
      />
