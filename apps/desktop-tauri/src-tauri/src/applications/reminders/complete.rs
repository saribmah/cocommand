//! Reminders complete tool.
//!
//! This module provides the tool for marking reminders as completed.
...
/// Tool ID for the complete tool.
pub const TOOL_ID: &str = "reminders_complete";

/// Tool for marking a reminder as completed.
pub struct RemindersComplete;

impl Tool for RemindersComplete {
    fn id(&self) -> &str {
        TOOL_ID
    }
    ...
    fn description(&self) -> &str {
        "Mark a reminder as completed in Apple Reminders. Searches for reminders whose name contains the query and marks the first match as done. Case-insensitive search."
    }
    ...
    fn schema(&self) -> Option<Value> {
        Some(json!({
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Search query to find the reminder to complete. Matches reminders whose name contains this text (case-insensitive)."
                },
                "list": {
                    "type": "string",
    ...
    fn execute(&self, inputs: Value) -> ToolResult {
        ...
        if query.is_empty() {
            return ToolResult::error("Search query is required to find the reminder to complete");
        }
        ...
        let script = build_complete_script(&escaped_query, list);

        run_applescript(&script)
    }
}

fn build_complete_script(query: &str, list: Option<&str>) -> String {
    ...
    // AppleScript to find and complete a reminder matching the query
    format!(
        r#"
tell application "Reminders"
    ...
    if foundReminder is missing value then
        ...
        return "Reminder '" & foundName & "' marked as completed."
    end if
end tell
"#,
...
mod tests {
    ...
    #[test]
    fn test_tool_id() {
        let tool = RemindersComplete;
        assert_eq!(tool.id(), "reminders_complete");
    }

    #[test]
    fn test_tool_name() {
        let tool = RemindersComplete;
        assert_eq!(tool.name(), "Complete Reminder");
    }
    ...
    fn test_build_complete_script() {
        let script = build_complete_script("pay rent", None);
        assert!(script.contains("pay rent"));
        assert!(script.contains("set completed of foundReminder to true"));
        assert!(script.contains("reminders whose completed is false"));
    }

    #[test]
    fn test_build_complete_script_with_list() {
        let script = build_complete_script("pay rent", Some("Bills"));
        assert!(script.contains("pay rent"));
