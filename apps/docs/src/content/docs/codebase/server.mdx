...
- `POST /sessions/command` -> store user message, run LLM, store assistant response.
- `GET /sessions/context` -> current session metadata.
- `GET /workspace/extensions` -> list all extensions.
- `POST /workspace/extensions/open` -> open extension and mark it active in the session.
