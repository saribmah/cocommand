...
2. Tauri boots the Rust backend server (`crates/cocommand`) and exposes `get_server_info_cmd` to the UI.
3. The React UI loads and fetches server info, then uses HTTP to talk to the backend.
